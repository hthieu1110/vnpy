...
class RecorderEngine(BaseEngine):
    ...
    def __init__(self, main_engine: MainEngine, event_engine: EventEngine) -> None:
        ...
        self.filter_dt: datetime = datetime.now(
            DB_TZ
        )  # Timestamp for tick data filtering
        self.filter_window: int = (
            60  # Time window for tick data filtering, default 60 seconds
        )
        self.filter_delta: timedelta  # Time delta object for tick data filtering
    ...
    def run(self) -> None:
        ...
        while self.active:
            try:
                ...
                self.write_log(f"Exception triggered, recording stopped:\n{info}")
    ...
    def add_bar_recording(self, vt_symbol: str) -> None:
        ...
        if vt_symbol in self.bar_recordings:
            self.write_log(f"Already in bar recording list: {vt_symbol}")
            return
        ...
        self.write_log(f"Successfully added bar recording: {vt_symbol}")

    def add_tick_recording(self, vt_symbol: str) -> None:
        """"""
        if vt_symbol in self.tick_recordings:
            self.write_log(f"Already in tick recording list: {vt_symbol}")
            return
        ...
        self.write_log(f"Successfully added tick recording: {vt_symbol}")

    def remove_bar_recording(self, vt_symbol: str) -> None:
        """"""
        if vt_symbol not in self.bar_recordings:
            self.write_log(f"Not in bar recording list: {vt_symbol}")
            return
        ...
        self.write_log(f"Successfully removed bar recording: {vt_symbol}")

    def remove_tick_recording(self, vt_symbol: str) -> None:
        """"""
        if vt_symbol not in self.tick_recordings:
            self.write_log(f"Not in tick recording list: {vt_symbol}")
            return
        ...
        self.write_log(f"Successfully removed tick recording: {vt_symbol}")
    ...
    def update_tick(self, tick: TickData) -> None:
        """"""
        # Filter tick data that deviates too much from local timestamp
