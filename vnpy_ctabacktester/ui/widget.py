...
class BacktesterManager(QtWidgets.QWidget):
    ...
    def init_ui(self) -> None:
        """"""
        self.setWindowTitle(_("CTA Backtesting"))
        ...
        self.daily_button: QtWidgets.QPushButton = QtWidgets.QPushButton(_("Daily P&L"))
        ...
        form.addRow(_("Trading Strategy"), self.class_combo)
        form.addRow(_("Symbol"), self.symbol_line)
        form.addRow(_("K-line Period"), self.interval_combo)
        form.addRow(_("Start Date"), self.start_date_edit)
        form.addRow(_("End Date"), self.end_date_edit)
        form.addRow(_("Commission Rate"), self.rate_line)
        form.addRow(_("Trading Slippage"), self.slippage_line)
        form.addRow(_("Contract Multiplier"), self.size_line)
        form.addRow(_("Price Tick"), self.pricetick_line)
        form.addRow(_("Backtesting Capital"), self.capital_line)
    ...
    def process_optimization_finished_event(self, event: Event) -> None:
        """"""
        self.write_log(_("Please click [Optimization Results] button to view"))
    ...
    def start_backtesting(self) -> None:
        ...
        if not class_name:
            self.write_log(_("Please select a strategy to backtest"))
            return
        ...
        if "." not in vt_symbol:
            self.write_log(_("Symbol missing exchange suffix, please check"))
            return

        __, exchange_str = vt_symbol.split(".")
        if exchange_str not in Exchange.__members__:
            self.write_log(_("Symbol exchange suffix is incorrect, please check"))
            return
...
class OptimizationSettingEditor(QtWidgets.QDialog):
    ...
    def init_ui(self) -> None:
        ...
        grid.addWidget(QLabel(_("Process Limit")), 1, 0)
        grid.addWidget(self.worker_spin, 1, 1, 1, 3)
        grid.addWidget(QLabel(_("Parameter")), 2, 0)
        grid.addWidget(QLabel(_("Start")), 2, 1)
        grid.addWidget(QLabel(_("Step")), 2, 2)
...
class OptimizationResultMonitor(QtWidgets.QDialog):
    ...
    def init_ui(self) -> None:
        """"""
        self.setWindowTitle(_("Parameter Optimization Results"))
        ...
        table.setHorizontalHeaderLabels([_("Parameter"), self.target_display])
        ...
        # Create layout
        button: QtWidgets.QPushButton = QtWidgets.QPushButton(_("Save"))
    ...
    def save_csv(self) -> None:
        ...
        with open(path, "w") as f:
            ...
            writer.writerow([_("Parameter"), self.target_display])
...
class BacktestingTradeMonitor(BaseMonitor):
    ...
    headers: dict = {
        ...
        "datetime": {"display": _("Time"), "cell": BaseCell, "update": False},
...
class BacktestingOrderMonitor(BaseMonitor):
    ...
    headers: dict = {
        ...
        "traded": {"display": _("Traded"), "cell": BaseCell, "update": False},
        "status": {"display": _("Status"), "cell": EnumCell, "update": False},
        "datetime": {"display": _("Time"), "cell": BaseCell, "update": False},
        "gateway_name": {"display": _("Gateway"), "cell": BaseCell, "update": False},
    }
...
class CandleChartDialog(QtWidgets.QDialog):
    ...
    def init_ui(self) -> None:
        """"""
        self.setWindowTitle(_("Backtesting Candlestick Chart"))
