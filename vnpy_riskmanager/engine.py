...
class RiskEngine(BaseEngine):
    ...
    def patch_send_order(self) -> None:
        ...
        """
        self._send_order: Callable[[OrderRequest, str], str] = (
    ...
    def update_setting(self, setting: dict) -> None:
        ...
        if self.active:
            self.write_log("Trading risk control started")
        else:
            self.write_log("Trading risk control stopped")
    ...
    def check_risk(self, req: OrderRequest, gateway_name: str) -> bool:
        ...
        if self.trade_count >= self.trade_limit:
            self.write_log(
                f"Daily total trade volume {self.trade_count} exceeds limit {self.trade_limit}"
            )
            return False
        ...
        if order_cancel_count >= self.order_cancel_limit:
            self.write_log(
                f"Daily {req.vt_symbol} cancel count {order_cancel_count} exceeds limit {self.order_cancel_limit}"
            )
            return False
        ...
        if req.direction == Direction.LONG:
            ...
            if best_ask and req.price >= best_ask:
                self.write_log(
                    f"Buy price {req.price} is greater than or equal to lowest ask price {best_ask}, may cause self-trading"
                )
                return False
        else:
            ...
            if best_bid and req.price <= best_bid:
                self.write_log(
                    f"Sell price {req.price} is less than or equal to highest bid price {best_bid}, may cause self-trading"
                )
                return False
    ...
    def get_order_book(self, vt_symbol: str) -> "ActiveOrderBook":
        """"""
        order_book: ActiveOrderBook | None = self.active_order_books.get(
            vt_symbol, None
        )
        if not order_book:
