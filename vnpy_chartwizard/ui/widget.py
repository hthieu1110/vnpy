...
class ChartWizardWidget(QtWidgets.QWidget):
    """Chart widget for candlestick charts"""
    ...
    def init_ui(self) -> None:
        ...
        hbox.addWidget(QtWidgets.QLabel("Symbol"))
    ...
    def new_chart(self) -> None:
        ...
        if not vt_symbol:
            self.main_engine.write_log("symbol is empty")
            return

        if vt_symbol in self.charts:
            self.main_engine.write_log("chart already exists")
            return

        if "LOCAL" not in vt_symbol:
            contract: ContractData | None = self.main_engine.get_contract(vt_symbol)
            if not contract:
                self.main_engine.write_log("contract not found")
                return
