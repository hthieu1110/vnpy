...
export const BacktestForm = (props: BacktestFormProps) => {
  ...
  useEffect(() => {
    ...
    const strategySettings = strategyParams ? JSON.stringify(strategyParams) : '';
    ...
    return [
      {
        ...
        options: strategies.filter((strategy) => strategy.strategy_name.startsWith('My')),
      },
      {
    ...
    >
      ...
      <div style={{ display: 'flex', flexDirection: 'row', gap: 16 }}>
            ...
            {strategiesGroups.map((group) => (
              <Select.OptGroup key={group.label} label={group.label}>
                {group.options.map((strategy) => (
                  <Select.Option key={strategy.strategy_name} value={strategy.strategy_name}>
        ...
        <Form.Item label='Slippage' name='slippage' rules={[{ required: true, message: 'Please input price!' }]}>
          <InputNumber placeholder='Slippage' min={0} style={{ width: 70 }} controls={false} />
        </Form.Item>

        <Form.Item label='Size' name='size' rules={[{ required: true, message: 'Please input size!' }]}>
          <InputNumber placeholder='Size' min={0} style={{ width: 70 }} controls={false} />
        </Form.Item>
        ...
        <div style={{ marginTop: 8 }}>
          <Form.Item name='strategySettings' rules={[{ required: true }]}>
            <Input
              ...
              onChange={(e) => {
                backtesterActions.updateParams({ strategySettings: e.target.value });
              }}
              style={{ width: 825 }}
            ></Input>
          </Form.Item>
